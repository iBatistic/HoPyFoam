...
__all__ = ['surfaceVectorBarGradField']
...
class surfaceVectorBarGradField(surfaceField):
    # Field dimensions
    _dimensions = 3
    ...
    def __init__(self, fieldName, baseField, baseFieldF):
        ...
        self._cellGradValues = self.calcCellGrad()

        # Interpolate cell centre values to face Gauss points
        self._faceGradValues = self.interpolateCellGradToFace()
    ...
    def calcCellGrad(self):
        ...
        for patch in psi._boundaryConditionsDict:
            ...
            patchContribution = eval("surfaceFieldBoundaryConditions.evaluateBarGradCellGrad." + patchType)
            patchContribution(self, psi, psif, patch, cellsGradValue)

        return cellsGradValue

    def interpolateCellGradToFace(self):
        mesh = self._baseField._mesh
        psi = self._baseField
        psif = self._baseFieldF
        cellGrad = self._cellGradValues
        ...
        for patch in psi._boundaryConditionsDict:
            ...
            # Call corresponding patch function
            patchContribution = eval("surfaceFieldBoundaryConditions.evaluateBarGrad." + patchType)
            patchContribution(self, psi, psif, cellGrad, patch, GaussPointsValues)
