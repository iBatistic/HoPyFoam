...
with open(fileName, 'r') as file:
    for line in file:
        parts = line.split()
        grid_size.append(math.sqrt(float(parts[0])))
        error_values.append(float(parts[2]))
...
L_2 = order(grid_size, 2, 1)#5
L_3 = order(grid_size, 3, 950)
L_4 = order(grid_size, 4, 180000)
plt.plot( grid_size, L_2, '--', color = "orange", linewidth = 1, label='Theoretical slope - 2 order')
#plt.plot( grid_size, L_3, '--', color = 'orange', linewidth = 1, label='Theoretical slope - 3 order')
#plt.plot( grid_size, L_4, '--', color = 'orange',  linewidth = 1, label='Theoretical slope - 4 order')

plt.legend()

figure = plt.gcf()
figure.savefig("absErrorConvergence.png", dpi=100)
plt.close(fig)
